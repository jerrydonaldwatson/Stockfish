...
Score Entry::do_king_safety(const Position& pos, Square ksq) {
  ...
  Value bonus1 = shelter_storm<Us>(pos, ksq);
  Value bonus2 = bonus1;

  // If we can castle use the bonus after the castling if it is bigger
  if (pos.can_castle(MakeCastling<Us, KING_SIDE>::right))
      bonus2 = std::max(bonus2, shelter_storm<Us>(pos, relative_square(Us, SQ_G1)));

  if (pos.can_castle(MakeCastling<Us, QUEEN_SIDE>::right))
      bonus2 = std::max(bonus2, shelter_storm<Us>(pos, relative_square(Us, SQ_C1)));

  castlingDifferential[Us] = (bonus2 - bonus1);
  
  return make_score(bonus1, -16 * minKingPawnDistance);
}
