...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    bool captureOrPromotion, doFullDepthSearch, moveCountPruning, lmrCaptures, skipQuiets, ttCapture, pvExact;
    Piece movedPiece;
...
moves_loop: // When in check, search starts from here
    ...
    while ((move = mp.next_move(skipQuiets)) != MOVE_NONE)
    {
      ...
      if (    depth >= 3 * ONE_PLY
          &&  moveCount > 1
          && (!captureOrPromotion || lmrCaptures))
      {
